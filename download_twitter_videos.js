...
(function() {
    ...
    function getRetweetFrame(tweetElement) {
        ...
        candidates.forEach((candidate) => {
            ...
            if (candidateFrame) {retweetFrame = candidateFrame;};
        });
    ...
    async function downloadFile(button, url, mode, filename) {
        ...
        GM.xmlHttpRequest({
            ...
            onprogress: function(progressEvent) {
                ...
                } else{button.innerText = 'Downloading...';}
            }
        });
    }
...
function createDownloadButtons(tweetElement) {
    // Check if this tweet has already been processed to avoid adding buttons again.
    if (tweetElement.hasAttribute('data-twitter-dl')) {
        return;
    }

    // Proceed with fetching media information for the tweet.
    ...
    getMediasFromTweetId(tweetInformation).then((medias) => {
        ...
        // At this point, we have verified that there are media links to create buttons for.
        // Check again for an extremely rare case where the attribute might have been set
        // after the async call started but before it completed.
        if (tweetElement.hasAttribute('data-twitter-dl')) {
            return;
        }
        ...
        let lowQualityButton, highQualityButton;

        //if (medias.lq) {
        //    lowQualityButton = createDownloadButton(tweetInformation, medias.lq, tweetInformation.isGif ? "gif" : "lq");
        //}
        if (medias.hq && !tweetInformation.isGif) {
            highQualityButton = createDownloadButton(tweetInformation, medias.hq, "hq");
        }
        ...
        // Now we're ready to add the buttons. Inject them into the DOM.
        // if (lowQualityButton) topBar.appendChild(lowQualityButton);
        if (highQualityButton) topBar.appendChild(highQualityButton);

        // Finally, mark the tweet as processed to prevent duplicate injections.
        tweetElement.setAttribute('data-twitter-dl', 'injected');
    }).catch(error => console.error('Error fetching media information:', error));
}
    ...
    function getTweetInformation(tweetElement) {
        ...
        try {
            if (isRetweet) {
                if (isPost) {
                    ...
                    if (hasRetweetVideoPlayer){fallback();}
                } else fallback();
            } else {
                const timeElement = tweetElement.querySelector("time");
        ...
        if (spanElement){isGif = spanElement.innerText == "GIF";}
        if (!id) return;
        information.id = id;
    ...
    async function injectAll(){
        ...
        for (let i = 0; i < tweets.length; i++) {
            ...
            // Check if the tweet already has download buttons to prevent adding them again
            if (tweet.hasAttribute('data-twitter-dl')) {
                console.log("skipping tweet")
                continue; // This tweet has already been processed, skip it
            }
            const alreadyHasDownloadButtons = tweet.querySelector(".dl-video");
            if (alreadyHasDownloadButtons) {
                console.log("skipping tweet")
                continue; // Skip this tweet as it already has download buttons
            }
